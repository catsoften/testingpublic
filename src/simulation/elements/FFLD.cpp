...
static int update(UPDATE_FUNC_ARGS)
{
	...
	{
		sim->gravIn.mass[Vec2{ x, y } / CELL] = -3.0f;
	}
	else // Negate local gravity
	{
		sim->gravIn.mass[Vec2{ x, y } / CELL] = 0;
	}
	...
	for (auto rx = -3; rx <= 3; rx++)
	{
		for (auto ry = -3; ry <= 3; ry++)
		{
			...
			if (!r || TYP(r) == PT_FFLD || TYP(r) == PT_FFGN)
			{
				continue;
			}
			...
			if (TYP(r) == PT_VIRS || TYP(r) == PT_VRSG || TYP(r) == PT_VRSS) // Cure VIRS
			{
				parts[ID(r)].tmp3 = 8;
			}
...
static int graphics(GRAPHICS_FUNC_ARGS)
{
	...
	auto value = gfctx.rng.between(0, 100) + 90;
	*colr = value;
	*colg = value;
	*colb = gfctx.rng.between(0, 55) + 200;
