...
static int update(UPDATE_FUNC_ARGS)
{
	...
	for (auto rx = -1; rx <= 1; rx++)
	{
		for (auto ry = -1; ry <= 1; ry++)
		{
			if (rx || ry)
			{
				...
				if (rt == PT_HONY || rt == PT_SEED || rt == PT_SUGR || rt == PT_SWTR) // Eat sugar, HONY and SEED
				{
					sim->kill_part(ID(r));
				}
				else if (elements[rt].Properties & TYPE_LIQUID) // Drown
				{
					sim->kill_part(i);
					return 1;
				}
	...
	// Turn 180 if into a wall or will cause excessive stacking
	if (TYP(r) != PT_SOIL || sim->pmap_count[y + ry][x + rx] >= 3)
	{
	...
	{
		parts[ID(r)].tmp2 = 2;
		parts[i].tmp = (parts[i].tmp + (sim->rng.chance(1, 2) ? 3 : 5)) % 4;
	}
