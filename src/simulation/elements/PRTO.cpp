		...
		if (rx || ry)
		{
			...
			if (!r)
			{
				...
				for (auto nnx =0 ; nnx<80; nnx++)
				{
					...
					if (sim->portalp[channel][randomness][nnx].type==PT_SPRK)// TODO: make it look better, spark creation
					{
						...
						sim->create_part(-1,x-1,y,PT_SPRK);
						sim->create_part(-1,x-1,y-1,PT_SPRK);
						memset(&sim->portalp[channel][randomness][nnx], 0, sizeof(Particle));
						break;
					}
					else if (sim->portalp[channel][randomness][nnx].type)
					{
						if (sim->portalp[channel][randomness][nnx].type==PT_STKM)
							sim->player.spwn = 0;
						if (sim->portalp[channel][randomness][nnx].type==PT_STKM2)
							sim->player2.spwn = 0;
						if (sim->portalp[channel][randomness][nnx].type==PT_FIGH)
						{
							sim->fighcount--;
							sim->fighters[(unsigned char)sim->portalp[channel][randomness][nnx].tmp].spwn = 0;
						}
						auto np = sim->create_part(-1, x+rx, y+ry, sim->portalp[channel][randomness][nnx].type);
						if (np<0)
						{
							if (sim->portalp[channel][randomness][nnx].type==PT_STKM)
								sim->player.spwn = 1;
							...
							{
								sim->fighcount++;
								sim->fighters[(unsigned char)sim->portalp[channel][randomness][nnx].tmp].spwn = 1;
							}
							continue;
						}
						...
						if (sim->portalp[channel][randomness][nnx].vx == 0.0f && sim->portalp[channel][randomness][nnx].vy == 0.0f)
						{
							...
							parts[np] = sim->portalp[channel][randomness][nnx];
							parts[np].vx = tmp_vx;
							parts[np].vy = tmp_vy;
						}
						else
