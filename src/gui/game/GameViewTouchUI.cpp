...
// Begin main menu
	...
	tempButton->Appearance.Margin.Left += 2;
	tempButton->SetTogglable(true);
	tempButton->SetActionCallback({ [this] { rendererSettings->gridSize = rendererSettings->gridSize ? 0 : 1; } }); // Quickly toggle cell grid
	tempButton->SetStateFunction([this] { return rendererSettings->gridSize; });
	AddComponent(tempButton);
	mainMenuButtons.push_back(tempButton);
	...
	tempButton->SetStateFunction([this] { return (bool)(rendererSettings->findingElement); });
	AddComponent(tempButton);
	mainMenuButtons.push_back(tempButton);
	currentY -= 31;
	...
	authorshipInfoButton->SetIcon(IconIlluminati);
...
void GameViewTouchUI::NotifyActiveMenuToolListChanged(GameModel * sender)
{
