...
ElementSearchActivity::ElementSearchActivity(GameController * gameController, std::vector<Tool*> tools) :
	WindowActivity(ui::Point(-1, -1), TOUCH_UI ? ui::Point(XRES-71, YRES-71) : ui::Point(236, 302)),
	firstResult(NULL),
	...
	exit(false)
{
	...
	searchField = new ui::Textbox(ui::Point(8, 23), ui::Point(Size.X - (TOUCH_UI ? 72 : 16), 17), "");
	...
	scrollPanel = new ui::ScrollPanel(
		...
		Size - Vec2{18, searchField->Position.Y + searchField->Size.Y + (TOUCH_UI ? 18 : 33)}
	);
	...
	{
		closeButton = new ui::Button(ui::Point(Size.X - 56, 8), ui::Point(48, 32), "Close");
	}
	else
	{
...
{
	position += TOUCH_UI ? Vec2{3, 3} : Vec2{1, 1};

	std::unique_ptr<VideoBuffer> tempTexture = tool->GetTexture(ToolButton::DEFAULT_SIZE - Vec2{4, 4});
	ToolButton * tempButton;

	if(tempTexture)
		tempButton = new ToolButton(position, ToolButton::DEFAULT_SIZE, "", tool->Identifier, tool->Description);
	else
		tempButton = new ToolButton(position, ToolButton::DEFAULT_SIZE, tool->Name, tool->Identifier, tool->Description);
...
void ElementSearchActivity::PlaceTools() // Touch UI element selector layout
{
	...
	for (size_t i = 0; i < menuList.size(); i++)
	{
		...
		{
			current.Y += 5 + (current.X ? ToolButton::DEFAULT_SIZE.Y : 0);
			current.X = 0;

			ui::Label * tempLabel = new ui::Label(ui::Point(4, current.Y - 2), ui::Point(12, 15), menu->GetIcon());
			scrollPanel->AddChild(tempLabel);
			menuLabels.push_back(tempLabel);

			tempLabel = new ui::Label(ui::Point(16, current.Y), ui::Point(scrollPanel->Size.X-20, 15), menu->GetDescription());
			...
			for (auto tool : menu->GetToolList())
			{
				...
				current.X += ToolButton::DEFAULT_SIZE.X + 1;

				if(current.X + ToolButton::DEFAULT_SIZE.X > scrollPanel->Size.X)
				{
					current.X = 0;
					current.Y += ToolButton::DEFAULT_SIZE.Y + 1;
				}
			}
		}
	}
	scrollPanel->InnerSize = ui::Point(scrollPanel->Size.X, current.Y + ToolButton::DEFAULT_SIZE.Y + 6);
}
...
void ElementSearchActivity::SearchTools(String query)
{
	...
	for (auto &match : matches)
	{
		...
		if(current.Y + ToolButton::DEFAULT_SIZE.Y > scrollPanel->Size.Y)
			break;
	}
	scrollPanel->InnerSize = scrollPanel->Size;
}
